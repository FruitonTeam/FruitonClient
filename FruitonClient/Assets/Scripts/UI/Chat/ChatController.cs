    ...
    public class ChatController : MonoBehaviour, IOnItemSelectedListener, IOnMessageListener
    {
        ...
        public void OnAddFriendClick()
        {
            ...
            PlayerHelper.Exists(friendToAdd, exists =>
            {
                if (exists)
                {
                    PlayerHelper.GetAvatar(friendToAdd, texture =>
                    {
                        FriendListController.SetAvatar(friendToAdd, texture);
                    }, error =>
                    {
                        Debug.LogWarning("Could not get avatar for user " + friendToAdd + ". Default avatar will be used.");
                    });
